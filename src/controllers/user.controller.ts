...
export const loginUser = async (req: Request, res: Response) => {
  try {
    ...
    if (!currentUser.verified) {
      return res.status(400).json({ ok: false, msg: "Cuenta no verificada" });
    }

    const token = await generateJWT(currentUser.id, currentUser.username);
    return res
      .status(200)
      .json({ ok: true, msg: "Usuario logueado con éxito", token });
  } catch (error) {
    console.log(error);
